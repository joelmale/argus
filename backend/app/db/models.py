...
class Asset(Base):
    ...
    ttl_distance: Mapped[int | None] = mapped_column(Integer)
...
class ScanJob(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    parent_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("scan_jobs.id", ondelete="CASCADE"))
...
class ScannerConfig(Base):
    ...
    ai_backend: Mapped[str] = mapped_column(String(16), default="ollama")
    ai_model: Mapped[str | None] = mapped_column(String(128))
    fingerprint_ai_backend: Mapped[str] = mapped_column(String(16), default="ollama")
    ollama_base_url: Mapped[str | None] = mapped_column(Text)
